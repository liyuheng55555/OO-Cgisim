...
public class DrawController {
    ...
    private HashMap<Integer, MyShape> shapeMap = new HashMap<>();
    private int startID;
    private ArrayList<MyShape> list = new ArrayList<>();
    private final ArrayList<MyLine> listLine = new ArrayList<MyLine>();
    ...
    public HashMap<Integer, MyShape> getShapeMap(){
        for(MyLine lines : listLine) {
            MyShape start = lines.getHeadLinkShape();
            MyShape end = lines.getTailLinkShape();
            if(start != null && end != null){
                if(start instanceof StartNode){
                    ((StartNode) start).setNxtID(end.getFactoryID());
                }
                if(start instanceof IfNode){
                    if(lines.getSX() == start.getX() + start.getWidth()) {
                        ((IfNode) start).setTrueID(end.getFactoryID());
                    }else {
                        ((IfNode) start).setFalseID(end.getFactoryID());
                    }
                }
                if(start instanceof StatementNode){
                    ((StatementNode) start).setNxtID(end.getFactoryID());
                }
                if(end instanceof EndNode){
                    ((EndNode) end).setPreID(start.getFactoryID());
                }
                if(end instanceof StatementNode){
                    ((StatementNode) end).setPreID(start.getFactoryID());
                }
            }
        }
        for(MyShape shapes : list) {
            shapeMap.put(shapes.getFactoryID(),shapes);
        }
        return shapeMap;
    }
    ...
    public void saveChange() {
        ...
        System.out.println("one change saved");
        compiler.getTextArea().setText(code);
    }
